...
type TargetQuery struct {
	...
	withParent   *TargetQuery
	withChildren *TargetQuery
	withSteps    *StepQuery
...
func (tq *TargetQuery) QueryParent() *TargetQuery {
	query := (&TargetClient{config: tq.config}).Query()
...
func (tq *TargetQuery) QueryChildren() *TargetQuery {
	query := (&TargetClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (tq *TargetQuery) QuerySteps() *StepQuery {
	query := (&StepClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TargetQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (tq *TargetQuery) FirstIDX(ctx context.Context) uint64 {
...
func (tq *TargetQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (tq *TargetQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (tq *TargetQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (tq *TargetQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *TargetQuery) WithParent(opts ...func(*TargetQuery)) *TargetQuery {
	query := (&TargetClient{config: tq.config}).Query()
...
func (tq *TargetQuery) WithChildren(opts ...func(*TargetQuery)) *TargetQuery {
	query := (&TargetClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TargetQuery) WithSteps(opts ...func(*StepQuery)) *TargetQuery {
	query := (&StepClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TargetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Target, error) {
	...
	if query := tq.withParent; query != nil {
		if err := tq.loadParent(ctx, query, nodes, nil,
			func(n *Target, e *Target) { n.Edges.Parent = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withChildren; query != nil {
		if err := tq.loadChildren(ctx, query, nodes,
			func(n *Target) { n.Edges.Children = []*Target{} },
			func(n *Target, e *Target) { n.Edges.Children = append(n.Edges.Children, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withSteps; query != nil {
		if err := tq.loadSteps(ctx, query, nodes,
			func(n *Target) { n.Edges.Steps = []*Step{} },
			func(n *Target, e *Step) { n.Edges.Steps = append(n.Edges.Steps, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TargetQuery) loadParent(ctx context.Context, query *TargetQuery, nodes []*Target, init func(*Target), assign func(*Target, *Target)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Target)
	for i := range nodes {
		fk := nodes[i].ParentID
...
func (tq *TargetQuery) loadChildren(ctx context.Context, query *TargetQuery, nodes []*Target, init func(*Target), assign func(*Target, *Target)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(target.FieldParentID)
	}
	query.Where(predicate.Target(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(target.ChildrenColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (tq *TargetQuery) loadSteps(ctx context.Context, query *StepQuery, nodes []*Target, init func(*Target), assign func(*Target, *Step)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Target)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(step.FieldRefTargetID)
	}
	query.Where(predicate.Step(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(target.StepsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "ref_target_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (tq *TargetQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(target.Table, target.Columns, sqlgraph.NewFieldSpec(target.FieldID, field.TypeUint64))
	_spec.From = tq.sql
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withParent != nil {
			_spec.Node.AddColumnOnce(target.FieldParentID)
		}
	}
